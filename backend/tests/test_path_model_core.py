...
from backend.app.path_dataset import frame_split
from backend.app.path_features import (
    RANDOM_FEATURES,
    RIDGE_LAMBDA,
    _random_projection,
    _standardize,
...
def test_feature_extraction_is_deterministic_and_finite():
    ...
    first = pixel_features(image)
    second = pixel_features(image)
...
def test_feature_vector_width_is_pinned_to_the_model_schema():
    ...
    assert pixel_features(_synthetic_frame(12, 16)).shape == (12 * 16, 22)
...
def test_classifier_learns_a_separable_split_and_scores_by_sign():
    ...
    model = fit_kernel_classifier(samples, labels, RANDOM_FEATURES, RIDGE_LAMBDA, seed=42)
    scores = predict_scores(samples, model)
...
def test_classifier_is_reproducible_for_a_fixed_seed():
    ...
    first = fit_kernel_classifier(samples, labels, RANDOM_FEATURES, RIDGE_LAMBDA, seed=42)
    same_seed = fit_kernel_classifier(samples, labels, RANDOM_FEATURES, RIDGE_LAMBDA, seed=42)
    other_seed = fit_kernel_classifier(samples, labels, RANDOM_FEATURES, RIDGE_LAMBDA, seed=43)
...
def test_chunked_prediction_matches_a_single_pass():
    ...
    whole = predict_scores(samples, model, chunk_size=len(samples))
    chunked = predict_scores(samples, model, chunk_size=64)
...
def test_cleanup_drops_isolated_pixels_but_keeps_connected_path():
    ...
    assert clean_prediction(noise.reshape(-1), (20, 20), 0.5).sum() == 0
    assert clean_prediction(block.reshape(-1), (20, 20), 0.5)[6:11, 6:11].all()
...
def test_frame_split_holds_back_one_frame_per_short_video():
    ...
    train, validation = frame_split(records)
...
def test_frame_split_never_validates_on_a_single_labelled_frame():
    train, validation = frame_split([{"video_id": "v1", "frame_index": 0}])
...
def test_rle_survives_an_encode_decode_round_trip():
    ...
    record = {"width": 3, "height": 2, "rle": encode_binary_rle(mask)}

    assert np.array_equal(decode_rle(record), mask)
...
def test_vectorized_rle_matches_naive_reference_and_handles_edges():
    ...
    assert encode_binary_rle(np.empty((0,), np.uint8)) == []
    assert encode_binary_rle(np.ones((4, 4), np.uint8)) == [1, 16]
...
def test_fused_score_path_matches_reference_formula():
    ...
    model = fit_kernel_classifier(samples, labels, 32, 0.05, seed=1)
    ...
    assert np.allclose(predict_scores(samples, model), reference, atol=1e-4)


def test_rle_with_a_truncated_run_is_rejected():
    with pytest.raises(ValueError):
        decode_rle({"width": 2, "height": 2, "rle": [0, 1]})
