...
from backend.app.path_masks import (
    apply_refinements,
    comparison_mask,
    confusion_counts,
    encode_binary_rle,
...
def test_polygon_becomes_binary_training_mask():
    ...
    mask = polygon_mask(record, 100, 60)
...
def test_comparison_mask_marks_correct_missed_and_invented_path():
    ...
    assert comparison_mask(truth, prediction).tolist() == [[1, 2], [3, 0]]
...
def test_saved_refinement_overrides_effective_training_truth(tmp_path):
    ...
    refined = apply_refinements(np.zeros((2, 2), dtype=np.uint8), tmp_path, "video-1", 7)
