...
const terrain = {
  ...
  ground: {mask: {width: 1, height: 1, rle: [1, 1]}, regions: [], confidence: .72, visible_ratio: .48, source: 'current_video_frame_inference'},
  traversability: {
  ...
  corridor: {status: 'uncertain', polygon: [], centerline: [], confidence: .61, minimum_width_ratio: .9, minimum_width_m: .68, stability_px: 2, stable_frames: 3, green_support_fraction: .5, source_frame_timestamp_ms: 0, temporally_smoothed: true, reasons: ['metric_scale_estimated']},
  factors: {free_width_score: .7, obstacle_clearance_score: .8, connectivity_score: .9, smoothness_score: .6, bottleneck_clearance_score: .7, visibility_score: .8, calibration_score: .45, temporal_stability_score: .9},
  quality: {blur_score: .1, exposure_score: .9, motion_inliers: 42, unknown_ratio: 0},
...
test('offers automatic and own-label overlays while keeping the safety boundary visible', async () => {
  render(<AnalysisView
    ...
    reconstruction={{
      traversals: [{video_id: 'video-1', video_name: 'Video 1', direction: 'A_TO_B', duration_seconds: 10, tracked_fraction: 1, median_matches: 10, median_inliers: 10, geojson: {type: 'LineString', coordinates: [[9.28, 48.73], [9.27, 48.74]]}}],
    } as any}
    segmentation={{
      schema_version: '3.0', run_id: 'run-1', mission_id: 'mission-1',
      model: {adapter: 'ForestInstanceAdapter', model_id: 'forest', version: '3', hardware: 'CPU'},
      configuration: {analysis_hz: 4, input_width: 640, min_area_ratio: .004, confirmation_hits: 3, max_track_gap_frames: 2, confidence_meaning: 'proxy'},
      ontology: {},
      terrain_ontology: {
      ...
      vehicle_configuration: {width_m: .35, safety_margin_per_side_m: .2, required_width_m: .75, source: 'documented_default_assumption'},
      videos: [{video_id: 'video-1', video_name: 'Video 1', duration_seconds: 10, fps: 30, width: 640, height: 360, analysis_interval_ms: 250, frames: [{video_id: 'video-1', video_name: 'Video 1', frame_index: 0, timestamp_ms: 0, quality: {sharpness: 100, motion_inliers: 42}, detections: [], terrain}], tracks: [], counts: {visible_individuals_latest_frame: 0, confirmed_unique_per_video: {tree: 0, shrub: 0}}, metrics: {analyzed_frames: 1, raw_detections: 0, tracks: 0, confirmed_tree_instances: 0, confirmed_shrub_instances: 0, average_track_length_frames: 0, short_track_fraction: 0, empty_frame_fraction: 1, median_motion_inliers: 42, inference_seconds: 1}}],
      counts: {confirmed_unique_per_video_sum: {tree: 0, shrub: 0}, mission_unique: null, mission_unique_reason: ''},
      metrics: {runtime_seconds: 1, analyzed_frames: 1, raw_detections: 0, tracks: 0, confirmed_tree_instances: 0, confirmed_shrub_instances: 0, empty_frame_fraction: 1, average_track_length_frames: 0},
