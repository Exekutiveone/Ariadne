...
const emptySummary = (missionId: string): GroundTruthSummary => ({
  schema_version: '2.0', mission_id: missionId, ontology: {},
  counts: {total: 0, draft: 0, confirmed: 0, skipped: 0}, items: [],
})
...
export function normalizedPointFromBounds(clientX: number, clientY: number, bounds: Pick<DOMRect, 'left' | 'top' | 'width' | 'height'>): NormalizedPoint {
...
export default function GroundTruthLabeler({mission, onClose, onProcessingComplete, onMissionUpdated}: {mission: Mission; onClose: () => void; onProcessingComplete: () => void | Promise<void>; onMissionUpdated?: () => void | Promise<void>}) {
  ...
  const [refinementSelection, setRefinementSelection] = useState<{point: NormalizedPoint; kind: 'missed_label' | 'invented_path'} | null>(null)
  ...
  const selectedFrames = useMemo(() => activeVideo ? buildFrameSelection(activeVideo.total_frames, selectionMode, stride, targetCount) : [], [activeVideo, selectionMode, stride, targetCount])
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    getLabelingVideos(mission.id).then(result => {
      if (cancelled) return
      setVideos(result.videos)
      setActiveVideoId(result.videos[0]?.video_id ?? '')
    }).catch(error => setMessage(error instanceof Error ? error.message : 'Videos konnten nicht geladen werden')).finally(() => !cancelled && setLoading(false))
    return () => {cancelled = true}
  }, [mission.id])

  useEffect(() => {
    void listGroundTruth(mission.id).then(setSummary).catch(error => setMessage(error instanceof Error ? error.message : 'Labelübersicht konnte nicht geladen werden'))
    void getPathModel(mission.id).then(setPathModel).catch(error => setMessage(error instanceof Error ? error.message : 'CPU-Wegmodell konnte nicht geladen werden'))
    void getPathTrainingJob(mission.id).then(setTrainingJob).catch(() => undefined)
  }, [mission.id])

  useEffect(() => {
    if (!trainingJob || !['queued', 'running'].includes(trainingJob.status)) return
    const timer = window.setInterval(() => {
      void getPathTrainingJob(mission.id).then(job => {
        setTrainingJob(job)
  ...
  useEffect(() => {
    if (!showAiMask || !pathModel || !activeVideo) {setPathPrediction(null); return}
    let cancelled = false
    setPredictionLoading(true)
    void predictPathFrame(mission.id, activeVideo.video_id, frameIndex)
      .then(prediction => {if (!cancelled) setPathPrediction(prediction)})
      .catch(error => {if (!cancelled) {setPathPrediction(null); setMessage(error instanceof Error ? error.message : 'KI-Wegmaske konnte nicht berechnet werden')}})
  ...
  useEffect(() => {
    ...
    if (video) {
      ...
      if (Math.abs(video.currentTime - seek) > .0005) video.currentTime = seek
    }
    let cancelled = false
    setLoading(true)
    setMessage('Frame wird geladen …')
    getGroundTruth(mission.id, activeVideo.video_id, frameIndex).then(annotation => {
      if (cancelled) return
  ...
  const pointerDown = (event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    if (refinementMode) {
      ...
      if (!comparison) {setMessage('Für diesen Frame ist keine farbige Modellbewertung verfügbar.'); return}
      const value = rleValueAt(comparison, point)
      if (value !== 2 && value !== 3) {setRefinementSelection(null); setMessage('Klicke eine rote oder gelbe Fehlerfläche an.'); return}
      setRefinementSelection({point, kind: value === 2 ? 'missed_label' : 'invented_path'})
      setMessage(value === 2 ? 'Rote Fläche ausgewählt: Das Modell hat deinen bisherigen Weg hier übersehen.' : 'Gelbe Fläche ausgewählt: Das Modell hat hier zusätzlichen Weg erkannt.')
      return
    }
  ...
  const zoomAt = (nextZoom: number, clientX?: number, clientY?: number) => {
    const bounded = clamp(nextZoom, .5, 6)
  ...
  const wheel = (event: ReactWheelEvent<HTMLDivElement>) => {event.preventDefault(); zoomAt(zoom * (event.deltaY < 0 ? 1.15 : 1 / 1.15), event.clientX, event.clientY)}

  const navigate = (direction: -1 | 1, allowDirty = false, carryForward = true) => {
    if (dirty && !allowDirty) {setMessage('Speichere die Änderung oder nutze Rückgängig, bevor du den Frame wechselst.'); return false}
    ...
    carryRef.current = carriedPoints && activeVideo ? {videoId: activeVideo.video_id, frameIndex: selectedFrames[nextPosition], points: carriedPoints} : null
  ...
  const skipFrame = async () => {
    if (dirty) {setMessage('Der Frame enthält ungespeicherte Änderungen. Speichere oder mache sie zuerst rückgängig.'); return}
    if (await persist('skipped')) navigate(1, true, false)
  }

  const openSavedMask = (item: GroundTruthSummary['items'][number]) => {
    if (dirty) {setMessage('Speichere oder verwirf zuerst die aktuelle Änderung, bevor du eine gespeicherte Maske öffnest.'); return}
  ...
  const startProcessing = async () => {
    if (!summary.counts.confirmed || processing) {setMessage('Bestätige mindestens ein Polygon, bevor du die Auswertung startest.'); return}
    setProcessing(true)
  ...
  const trainCpuPathModel = async () => {
    ...
    if (summary.counts.confirmed < 10) {setMessage('Für das Training werden mindestens 10 bestätigte Polygonframes benötigt.'); return}
    ...
    try {
      ...
      setPathModel(result)
      setMessage(`CPU-Training abgeschlossen: ${result.validation_metrics.symmetric_score.toFixed(2)} von 100 Punkten auf getrennten Validierungsframes.`)
    } catch (error) {
  ...
  const confirmRefinement = async () => {
    ...
    try {
      const saved = await savePathRefinement(mission.id, activeVideo.video_id, frameIndex, {x: refinementSelection.point[0], y: refinementSelection.point[1], expected_kind: refinementSelection.kind, action: 'accept_model'})
      ...
      setMessage(`Refinement gespeichert. Dieser Frame enthält jetzt ${saved.refinement_count} Feedback-Korrektur${saved.refinement_count === 1 ? '' : 'en'} für das nächste Training.`)
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Refinement konnte nicht gespeichert werden')
    } finally {setRefinementSaving(false)}
  }

  const startBackgroundTraining = async () => {
    if (dirty) {setMessage('Speichere oder verwirf zuerst die Änderungen am aktuellen Polygon.'); return}
    if (summary.counts.confirmed < 10) {setMessage('Für das Training werden mindestens 10 bestätigte Polygonframes benötigt.'); return}
    if (trainingJob && ['queued', 'running'].includes(trainingJob.status)) {setMessage('Für diese Mission läuft bereits ein Hintergrundtraining.'); return}
    try {
      const duration = trainingProfile === 'quick' ? .25 : trainingHours
  ...
  useEffect(() => {
    const keydown = (event: KeyboardEvent) => {
      ...
      else if (event.key === 'Delete' || event.key === 'Backspace') {event.preventDefault(); removeSelectedPoint()}
    }
    window.addEventListener('keydown', keydown)
    return () => window.removeEventListener('keydown', keydown)
  })

  if (!activeVideo) return <div className="labeling-page"><div className="labeling-header"><button onClick={onClose}>← Missionen</button><h1>Ground Truth</h1></div><div className="alert error">{loading ? 'Originalvideos werden gelesen …' : message || 'Kein Video verfügbar.'}</div></div>
  ...
  return <div className="labeling-page">
    <header className="labeling-header"><button onClick={onClose}>← Missionen</button><div><span className="eyebrow">MANUELLE GROUND TRUTH · OPTIONALE KI-VORSCHAU</span><h1>{mission.name}</h1><p>Markiere den befahrbaren Bereich im Originalvideoframe; die KI-Maske ist nur ein getrenntes Vergleichs-Overlay.</p></div><div className="labeling-progress"><b>Frame {frameIndex + 1} von {activeVideo.total_frames.toLocaleString('de-DE')}</b><span>Auswahl {selectionPosition + 1} von {selectedFrames.length.toLocaleString('de-DE')}</span></div></header>

    <div className="labeling-workspace">
      <section className="labeling-main">
        <div className="labeling-viewport-toolbar"><div><button className={isPlaying ? 'play-button active' : 'play-button'} disabled={dirty || selectedFrames.length < 2} onClick={() => setIsPlaying(current => !current)} aria-label={isPlaying ? 'Gelabelte Frames pausieren' : 'Gelabelte Frames abspielen'}>{isPlaying ? '❚❚ Pause' : '▶ Gelabelte Frames abspielen'}</button><label className="playback-speed-control">Tempo<select aria-label="Abspielgeschwindigkeit" value={playbackSpeed} onChange={event => setPlaybackSpeed(+event.target.value)}><option value="0.25">0,25×</option><option value="0.5">0,5×</option><option value="1">1×</option><option value="2">2×</option><option value="4">4×</option></select></label><button onClick={() => zoomAt(zoom / 1.25)} aria-label="Herauszoomen">−</button><b>{Math.round(zoom * 100)}%</b><button onClick={() => zoomAt(zoom * 1.25)} aria-label="Hineinzoomen">+</button><button onClick={resetViewport}>Ansicht zurücksetzen</button></div><span>Mausrad zoomt · Hand verschiebt das Bild</span></div>
        ...
        <div className="labeling-navigation"><button disabled={selectionPosition === 0 || saving || dirty} onClick={() => navigate(-1)}>← Vorheriger Frame</button><button disabled={saving || dirty} onClick={() => void skipFrame()}>Frame überspringen</button><button disabled={selectionPosition >= selectedFrames.length - 1 || saving || dirty} onClick={() => navigate(1)}>Nächster Frame →</button></div>
      </section>
      ...
      <aside className="labeling-controls">
        ...
        <label>Masken-Deckkraft · {Math.round(maskOpacity * 100)} %<input aria-label="Masken-Deckkraft" type="range" min="0" max="1" step="0.05" value={maskOpacity} onChange={event => setMaskOpacity(+event.target.value)}/></label>
    ...
    <section className="saved-mask-library">
      <div className="saved-mask-library-head"><div><span className="eyebrow">PERSISTENTE GROUND TRUTH</span><h2>Gespeicherte Polygonmasken</h2><p>Öffne einen Eintrag, um seine Maske wieder über dem Originalvideo anzuzeigen und weiterzubearbeiten.</p></div><b>{summary.counts.total} Einträge</b></div>
      {summary.items.length ? <div className="saved-mask-list">{summary.items.map(item => {
        const video = videos.find(candidate => candidate.video_id === item.video_id)
        return <button key={`${item.video_id}-${item.frame_index}`} className={item.video_id === activeVideo.video_id && item.frame_index === frameIndex ? 'active' : ''} onClick={() => openSavedMask(item)}>
          <span className={`mask-status ${item.status}`}>{item.status === 'confirmed' ? 'BESTÄTIGT' : item.status === 'skipped' ? 'ÜBERSPRUNGEN' : 'ENTWURF'}</span>
          <b>Frame {item.frame_index + 1}</b><span>{video?.original_name ?? item.video_id}</span><small>{item.statistics.polygon_count ?? 0} Polygon · {item.statistics.point_count ?? 0} Punkte · Revision {item.revision}</small>
        </button>
      })}</div> : <div className="empty saved-mask-empty">Noch keine Polygonmasken gespeichert. Sobald du einen Entwurf speicherst oder ein Polygon bestätigst, erscheint es hier.</div>}
    </section>

    <section className="path-model-panel">
      <div className="path-model-head"><div><span className="eyebrow">LOKALES CPU-TRAINING</span><h2>Wegerkennung aus deinen Labels</h2><p>Innerhalb deiner Polygone lernt das Modell „Weg“, außerhalb „kein Weg“. Übersehene und fälschlich erfundene Wegfläche kosten gleich viele Punkte.</p></div><button disabled={dirty || modelTraining || summary.counts.confirmed < 10} onClick={() => void trainCpuPathModel()}>{modelTraining ? 'CPU-TRAINING LÄUFT …' : `WEG-KI AUF ${summary.counts.confirmed} LABELFRAMES TRAINIEREN`}</button></div>
      <div className="background-training-panel">
        <div><b>Training im Hintergrund</b><span>Der Browser darf geschlossen werden. Der lokale Server und dieser Rechner müssen eingeschaltet bleiben.</span></div>
        <label>Profil<select aria-label="Trainingsprofil" value={trainingProfile} disabled={trainingJob?.status === 'running' || trainingJob?.status === 'queued'} onChange={event => setTrainingProfile(event.target.value as 'quick' | 'overnight')}><option value="quick">Schnelltest · 1 Kandidat</option><option value="overnight">Über Nacht · Varianten vergleichen</option></select></label>
        {trainingProfile === 'overnight' && <label>Dauer<select aria-label="Trainingsdauer" value={trainingHours} disabled={trainingJob?.status === 'running' || trainingJob?.status === 'queued'} onChange={event => setTrainingHours(+event.target.value)}><option value="1">1 Stunde</option><option value="4">4 Stunden</option><option value="8">8 Stunden</option><option value="12">12 Stunden</option></select></label>}
        <button disabled={dirty || summary.counts.confirmed < 10 || trainingJob?.status === 'running' || trainingJob?.status === 'queued'} onClick={() => void startBackgroundTraining()}>{trainingProfile === 'overnight' ? 'NACHTTRAINING STARTEN' : 'SCHNELLTEST IM HINTERGRUND'}</button>
      </div>
      {trainingJob && <div className={`training-job-status ${trainingJob.status}`} role="status"><b>{trainingJob.status === 'completed' ? 'Training abgeschlossen' : trainingJob.status === 'running' || trainingJob.status === 'queued' ? 'Training läuft' : 'Training unterbrochen'}</b><span>{trainingJob.message}</span><small>{trainingJob.candidates_completed} von {trainingJob.maximum_candidates} Kandidaten geprüft{trainingJob.best_validation_score !== null ? ` · bester Score ${trainingJob.best_validation_score.toFixed(2)} / 100` : ''}</small></div>}
      {pathModel ? <>
        <div className="active-model-version"><div><span>Aktives Modell</span><b>{pathModel.run_id}</b><small>Erstellt: {new Date(pathModel.created_at).toLocaleString('de-DE')}</small></div>{trainingJob?.status === 'completed' && trainingJob.initial_run_id && <strong className={trainingJob.best_run_id !== trainingJob.initial_run_id ? 'changed' : 'unchanged'}>{trainingJob.best_run_id !== trainingJob.initial_run_id ? '✓ MODELL AKTUALISIERT' : 'BISHERIGES MODELL BLEIBT BESSER'}</strong>}</div>
        <div className="path-model-metrics"><div><span>Validierungsscore</span><b>{pathModel.validation_metrics.symmetric_score.toFixed(2)} / 100</b></div><div><span>Punktabzug</span><b>{pathModel.validation_metrics.symmetric_penalty_points.toFixed(2)}</b></div><div><span>Weg übersehen</span><b>{Math.round(pathModel.validation_metrics.missed_label_fraction * 1000) / 10} %</b></div><div><span>Weg erfunden</span><b>{Math.round(pathModel.validation_metrics.invented_path_fraction * 1000) / 10} %</b></div><div><span>IoU</span><b>{Math.round(pathModel.validation_metrics.iou * 1000) / 1000}</b></div><div><span>CPU-Laufzeit</span><b>{pathModel.runtime_seconds.toFixed(1)} s</b></div></div>
        <div className="path-model-split"><b>{pathModel.ground_truth.confirmed_frames} Ground-Truth-Frames aus {pathModel.ground_truth.videos} Videos</b><span>{pathModel.split.train_frames} Training · {pathModel.split.validation_frames} getrennte Validierung · {pathModel.split.training_pixels_sampled.toLocaleString('de-DE')} Trainingspixel</span><small>Kein Frame befindet sich gleichzeitig in Training und Validierung. Keine Cloud und keine GPU verwendet.</small></div>
        <div className="path-model-evidence">{pathModel.evidence.map(item => <figure key={`${item.video_id}-${item.frame_index}`}><img src={item.image_url} alt={`${item.kind} Validierungsframe ${item.frame_index + 1}`}/><figcaption><b>{item.kind === 'best' ? 'Bester' : item.kind === 'worst' ? 'Schwierigster' : 'Mittlerer'} Validierungsframe</b><span>Frame {item.frame_index + 1} · Score {item.metrics.symmetric_score.toFixed(1)}</span><small>Grün korrekt · Rot übersehen · Gelb fälschlich erkannt</small></figcaption></figure>)}</div>
      </> : <div className="empty path-model-empty">Noch kein CPU-Wegmodell trainiert. Deine Polygonlabels bleiben dabei unverändert.</div>}
    </section>

    <section className="labeling-finish"><div><b>Markierungsrunde abschließen</b><span>Erst diese explizite Aktion startet die vollständige Segmentierung, Befahrbarkeit und Trajektorienberechnung. Die optionale türkisfarbene Wegmaske ist nur eine Modellvorschau und verändert deine Ground-Truth-Polygone nicht.</span></div><button disabled={processing || summary.counts.confirmed === 0 || dirty} onClick={() => void startProcessing()}>{processing ? 'AUSWERTUNG LÄUFT …' : 'MARKIERUNG ABSCHLIESSEN & AUSWERTUNG STARTEN'}</button></section>
  </div>
}
