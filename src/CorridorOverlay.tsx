 ...
 *  nicht verzerren. */
export default function CorridorOverlay({check, activeCorridor, onSelectCorridor, proposal, draft, planning, aspect, onMovePoint, onAddPoint, onRemovePoint}: {
  ...
  return <svg ref={surfaceRef} className={`corridor-overlay ${planning ? 'planning' : ''}`} viewBox="0 0 1 1" preserveAspectRatio="none" aria-label="Korridore und Trajektorie"
    onPointerMove={event => {
    ...
    <polyline points={polyline(decomposition.relevant_triangle_normalized)} className="corridor-triangle" vectorEffect="non-scaling-stroke"/>

    {corridors.map(corridor => <g key={corridor.corridor}
      className={`corridor-wedge ${corridor.status} ${corridor.corridor === activeCorridor ? 'active' : ''}`}
      onClick={event => {event.stopPropagation(); onSelectCorridor(corridor.corridor)}}>
    ...
    {draft?.map((point, index) => <ellipse key={index} cx={point[0]} cy={point[1]} rx={handleX} ry={handleY}
      className="corridor-handle"
      onPointerDown={event => {event.stopPropagation(); draggingRef.current = index; (event.target as Element).setPointerCapture?.(event.pointerId)}}
      onContextMenu={event => {event.preventDefault(); event.stopPropagation(); onRemovePoint(index)}}/>)}

    {/* Fluchtpunkt zuletzt, damit er immer obenauf liegt. */}
    <g className="corridor-vanishing">
      <line x1={vanishingX - .035} y1={vanishingY} x2={vanishingX + .035} y2={vanishingY} vectorEffect="non-scaling-stroke"/>
      <line x1={vanishingX} y1={vanishingY - .035 * aspect} x2={vanishingX} y2={vanishingY + .035 * aspect} vectorEffect="non-scaling-stroke"/>
      <ellipse cx={vanishingX} cy={vanishingY} rx={.014} ry={.014 * aspect} vectorEffect="non-scaling-stroke"/>
    </g>
  </svg>
}
