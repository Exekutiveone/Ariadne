...
import type {GlobalModelDashboardData, GlobalVideoAnalysisResult, GlobalVideoAnalysisStatus, LabelingVideo, PathPrediction} from './types'
...
export default function GlobalModelDashboard({onClose}: {onClose: () => void}) {
  ...
  useEffect(() => {
    if (!data?.model || !selectedMissionId || !activeVideo || analysisResult) {if (!analysisResult) setPrediction(null); return}
    let cancelled = false
    ...
    return () => {cancelled = true}
  }, [data?.model?.run_id, selectedMissionId, activeVideo?.video_id, frameIndex, analysisResult?.model_run_id])
  ...
  const displayed = analyzedFrame ? {mask: analyzedFrame.mask, evaluation: analyzedFrame.evaluation, path_fraction: analyzedFrame.path_fraction, model_run_id: analysisResult!.model_run_id} : prediction
  ...
  useEffect(() => {
    ...
    if (!context) return
    const mask = displayed.evaluation?.comparison_mask ?? displayed.mask
    ...
    for (let index = 0; index < mask.rle.length; index += 2) {
      const value = mask.rle[index], length = mask.rle[index + 1]
      if (value > 0) for (let offset = 0; offset < length; offset++) {
        const target = (pixel + offset) * 4
        let colour: number[] | null = null
        if (displayed.evaluation) {
          const belongsToAi = value === 1 || value === 3
          const belongsToLabel = value === 1 || value === 2
          if (showAiMask && showLabelMask) colour = value === 1 ? [58, 214, 92] : value === 2 ? [224, 74, 68] : [239, 196, 55]
          else if (showAiMask && belongsToAi) colour = [64, 220, 235]
          else if (showLabelMask && belongsToLabel) colour = [58, 214, 92]
        } else if (showAiMask) colour = [64, 220, 235]
        if (colour) {
          image.data[target] = colour[0]; image.data[target + 1] = colour[1]; image.data[target + 2] = colour[2]; image.data[target + 3] = 255
        }
      }
      pixel += length
    }
    context.putImageData(image, 0, 0)
  }, [displayed, showAiMask, showLabelMask])
  ...
  return <div className="global-model-page">
    ...
    <section className="global-analysis-player">
      ...
      {!model ? <div className="empty">Trainiere zuerst ein globales Modell, um den Analyseplayer zu aktivieren.</div> : <>
        ...
        <div className={`global-player-grid ${analysisResult ? 'ready' : 'waiting'}`}><div className="global-video-column"><div className="global-video-stage" style={activeVideo ? {aspectRatio: `${activeVideo.width}/${activeVideo.height}`} : undefined}>{activeVideo && <video ref={videoRef} src={`/api/v1/missions/${selectedMissionId}/videos/${activeVideo.video_id}/content`} muted playsInline preload="metadata" onTimeUpdate={event => {if (activeVideo) setFrameIndex(Math.min(activeVideo.total_frames - 1, Math.floor(event.currentTarget.currentTime * activeVideo.fps)))}} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onEnded={() => setPlaying(false)}/>}<canvas ref={canvasRef} style={{opacity: analysisResult ? maskOpacity : 0}}/>{!analysisResult && <div className="global-player-lock"><b>{analysisStatus?.status === 'running' ? `${Math.round(analysisStatus.progress * 100)} % analysiert` : 'Video noch nicht analysiert'}</b><span>Nach Abschluss kann das Video normal mit synchroner KI-Maske abgespielt werden.</span></div>}{predictionLoading && !analysisResult && <div className="global-prediction-loading">VORSCHAU WIRD BERECHNET …</div>}</div><div className="global-transport"><button disabled={!analysisResult || frameIndex <= 0} onClick={() => seekFrame(frameIndex - 1)}>←</button><button disabled={!analysisResult} onClick={togglePlayback}>{playing ? '❚❚ PAUSE' : '▶ PLAY'}</button><input aria-label="Globaler Analyseframe" type="range" min="0" max={Math.max(0, (activeVideo?.total_frames ?? 1) - 1)} value={frameIndex} disabled={!analysisResult} onChange={event => seekFrame(+event.target.value)}/><span>Frame {frameIndex + 1} / {(activeVideo?.total_frames ?? 0).toLocaleString('de-DE')}</span><button disabled={!analysisResult || !activeVideo || frameIndex >= activeVideo.total_frames - 1} onClick={() => seekFrame(frameIndex + 1)}>→</button></div></div><aside className="global-player-controls"><div className="global-mask-switches"><label><input type="checkbox" checked={showAiMask} disabled={!analysisResult} onChange={event => setShowAiMask(event.target.checked)}/><span>KI-Maske anzeigen</span></label><label><input type="checkbox" checked={showLabelMask} disabled={!analysisResult} onChange={event => setShowLabelMask(event.target.checked)}/><span>Label-Maske anzeigen</span></label></div><label>Abspieltempo<select value={speed} disabled={!analysisResult} onChange={event => setSpeed(+event.target.value)}><option value="0.25">0,25×</option><option value="0.5">0,5×</option><option value="1">1×</option><option value="2">2×</option></select></label><label>Masken-Deckkraft · {Math.round(maskOpacity * 100)} %<input type="range" min="0.05" max="0.9" step="0.05" value={maskOpacity} disabled={!analysisResult || (!showAiMask && !showLabelMask)} onChange={event => setMaskOpacity(+event.target.value)}/></label>{displayed && analysisResult && <div className="global-frame-result"><b>{displayed.evaluation ? `${displayed.evaluation.metrics.symmetric_score.toFixed(1)} / 100` : `${(displayed.path_fraction * 100).toFixed(1)} % Weg`}</b><span>{!showAiMask && !showLabelMask ? 'Originalvideo ohne Masken' : displayed.evaluation ? 'Gelabelter Frame: Grün korrekt · Rot übersehen · Gelb fälschlich' : 'Türkis: vorberechnete globale KI-Wegmaske'}</span><small>Modell {displayed.model_run_id}</small></div>}{analysisResult && <div className="global-analysis-complete"><b>Wiedergabe bereit</b><span>{analysisResult.analyzed_frames.toLocaleString('de-DE')} Frames in {duration(analysisResult.runtime_seconds)} analysiert.</span></div>}</aside></div>
      </>}
    </section>
  </div>
}
