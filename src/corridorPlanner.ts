...
import {deleteTrajectory, getCorridorCheck, getTrajectory, saveTrajectory} from './api'
...
/** Hält Korridorprüfung, Auswahl und die von Hand bearbeitete Trajektorie
 *  zusammen, damit Overlay und Bedienfeld denselben Zustand sehen. */
export function useCorridorPlanner(missionId: string, videoId: string, frameIndex: number, timestampMs: number, enabled: boolean) {
  ...
  const [calibration, setCalibration] = useState<CorridorCalibration>({vehicle_width_m: 1.2, clearance_m: .1, ground_width_at_bottom_m: 4})

  useEffect(() => {
    if (!enabled || !missionId || !videoId) {setCheck(null); return}
    let cancelled = false
    setLoading(true)
    void getCorridorCheck(missionId, videoId, frameIndex, calibration)
  ...
  }, [enabled, missionId, videoId, frameIndex, calibration])

  // Gespeicherte Trajektorie des Frames laden; sie hat Vorrang vor dem Vorschlag.
  useEffect(() => {
    if (!enabled || !missionId || !videoId) {setStored(null); setDraft(null); return}
    let cancelled = false
    void getTrajectory(missionId, videoId, frameIndex)
      ...
      .catch(() => {if (!cancelled) {setStored(null); setDraft(null)}})
    return () => {cancelled = true}
  ...
  const adopt = useCallback(() => {
    if (!corridorProposal.length) {setMessage('Für diesen Korridor gibt es keinen Vorschlag.'); return}
  ...
  const clear = useCallback(() => {setDraft(null); setMessage('Entwurf verworfen.')}, [])
  ...
  const persist = useCallback(async (note: string, annotator: string) => {
    ...
    try {
      ...
      setStored(saved)
      setMessage(`Trajektorie für Frame ${frameIndex + 1} gespeichert (Revision ${saved.revision}, ${unchangedFromProposal ? 'unverändert vom Modell' : 'von Hand nachgebessert'}).`)
    } catch (problem) {
  ...
  return {
    check, stored, draft, error, loading, saving, message, calibration, setCalibration,
    activeCorridor, setSelected, corridorProposal, unchangedFromProposal,
    adopt, clear, movePoint, addPoint, removePoint, persist, discard, setMessage,
  }
}
